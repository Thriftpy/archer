...
"""
    ...
    A simple command line application to run Archer apps.
...
def find_best_app(module):
    ...
    raise NoAppException('Failed to find application in module "%s".  Are '
                         'you sure it contains a Archer application?  Maybe '
...
class DispatchingApp(object):
    """Special application that dispatches to a Archer application which
...
class ScriptInfo(object):
    """Help object to deal with Archer applications.  This is usually not
    ...
    def load_app(self):
        """Loads the Archer app (if not yet loaded) and returns it.  Calling
        ...
        if self.create_app is not None:
            ...
            if self.app_import_path is None:
                raise NoAppException('Could not locate Archer application. '
                                     'You did not provide Archer_APP or the '
                                     '--app parameter.')
...
class AppGroup(click.Group):
    ...
    Not to be confused with :class:`ArcherGroup`.
    """
...
class ArcherGroup(AppGroup):
    """Special subclass of the :class:`AppGroup` group that supports
    loading more commands from the configured Archer app.  Normally a
    ...
    def main(self, *args, **kwargs):
        ...
        kwargs['obj'] = obj
        kwargs.setdefault('auto_envvar_prefix', 'Archer')
...
def run_command(info, host, port, reload, debugger, eager_loading,
                with_threads):
    """Runs a local development server for the Archer application.
    ...
    Archer is enabled and disabled otherwise.
    """
    ...
    if os.environ.get('WERKZEUG_RUN_MAIN') != 'true':
        ...
        if info.app_import_path is not None:
            print(' * Serving Archer app "%s"' % info.app_import_path)
...
def shell_command():
    ...
    Archer application.  The application will populate the default
...
cli = ArcherGroup(help="""\
This shell command acts as general utility script for Archer applications.

It loads the application configured (either through the Archer_APP environment
variable or the --app parameter) and then provides commands either provided
by the application or Archer itself.

The most useful commands are the "run" and "shell" command.

Example usage:

  Archer --app=hello --debug run
""")
...
def main(as_module=False):
    ...
    if as_module:
        ...
        # This module is always executed as "python -m Archer.run" and as such
